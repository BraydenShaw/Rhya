...
class SessionManager:
    ...
    async def process_message(self, context: ConnectionContext, message: dict):
        """处理用户消息，包括文本和附件"""
        ...
        if attachments:
            message_content += f"\n附件: {json.dumps(attachment_info, ensure_ascii=False)}"
        ...
        try:
            ...
            for attachment in attachments:
                if attachment.get("type", "").startswith("image/"):
                    # 对于图片附件，添加图片 URL 到 parts
                    parts.append(types.Part(
                        text=f"[图片附件] {attachment.get('name')} (URL: {attachment.get('url')})"
                    ))
                    logger.info(f"处理图片附件: {attachment.get('name')} - URL: {attachment.get('url')}")
    ...
    try:
        while True:
            ...
            if message_type == "message":
                ...
                if content:
                    await manager.process_message(context, data)
...
async def upload_file_to_oss(file: UploadFile = File(...)):
    """
    处理文件上传到阿里云 OSS，并返回文件 URL。
    """
    try:
        # 读取文件内容
        file_content = await file.read()
        
        # 生成唯一的 Object Key（使用 UUID + 原文件名，避免冲突）
        object_key = f"{uuid.uuid4().hex}_{file.filename}"
        
        # 上传到 OSS
        result = oss_bucket.put_object(object_key, file_content)
        
        if result.status != 200:
            raise Exception(f"OSS 上传失败，状态码: {result.status}")
        
        # 生成文件访问 URL（公开访问，假设 Bucket 是公开读的；如果私有，需要签名 URL）
        file_url = f"https://{OSS_BUCKET_NAME}.oss-cn-beijing.aliyuncs.com/{object_key}"
        
        logger.info(f"文件上传成功: {file.filename} -> {file_url}")
